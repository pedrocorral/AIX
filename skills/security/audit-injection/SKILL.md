...
Read: register rows for category INJ (`grep -n "VUL-INJ" docs/security/vulnerability-register.md`); `docs/meta-docs/security/secure-coding-baseline.md`; relevant meta-doc: `persistence/abstraction-layer.md`.

## Script first (near-zero tokens)
Run `bash <skill-dir>/scripts/scan.sh [paths]` first: it prints only suspicious file:line hits. Reason about the hits; do not read files that produced none. `<skill-dir>` is this skill's folder (e.g. `skills/security/audit-injection/` or the installed copy).
...
## Procedure
1. Locate code by layer (`docs/meta-docs/architecture/project-layout.md`): `backend/app/*/adapters/`, `core/`, any file/subprocess helpers. Grep targeted patterns; do not read whole directories.
