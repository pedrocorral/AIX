...
nesting depth, parameters, naming, docstring, magic numbers. Per file: length. Thresholds live in framework.yaml
(`style:` block) with defaults from the literature (docs/meta-docs/conventions/readability.md).
...
def thresholds():
    """framework.yaml `style:` block overrides DEFAULTS; missing keys keep the default."""
    out = dict(DEFAULTS)
    fy = ROOT / "framework.yaml"
    if fy.exists():
