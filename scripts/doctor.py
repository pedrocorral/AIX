...
def check_links():
    ...
    for name in off - set(cat):
        problem(f"disabled_skills names unknown skill {name}", "edit framework.yaml")
...
def check_extern():
    ext = ROOT / "skills" / "extern"
    for d in ext.iterdir() if ext.is_dir() else []:
        if d.is_dir() and (d / "SKILL.md").exists() and not (d / ".aix-source").exists():
            problem(f"skills/extern/{d.name} has no .aix-source (cannot `aix skills update` it)", "re-add it with `aix skills add`")
