...
├── README.md  CHANGELOG.md  LICENSE (MIT)  .aix/config.yaml (manifest: paths, id prefixes, markers)
...
│   ├── secretscan.py, secretrules.py   # leaves: the gitleaks logic (keyword gate, regex, entropy, allowlists) and its rule set, generated by tests/benchmark/gitleaks_rules.py
...
- Test suite: `aix self-test` (long form `python -m unittest discover -s tests`) (45 tests, ~25 s, temp folders only; `AIX_TEST_NETWORK=1` adds the registry downloads; no CI workflow: it runs when someone runs it). See `tests/README.md`. The manual smoke tests below are what the suite automates.
...
- A layer may also carry `templates/docs/` (files overlaid on the documentation seed, file by file, custom over org over kit) and `templates/pointers/` (the texts of `CLAUDE.md`, `GEMINI.md`, `copilot-instructions.md`, `aix.mdc`); everything a project receives comes from inside `.aix/` for that reason (ADR-0005).
