...
- Test suite: `aix self-test` (long form `python -m unittest discover -s tests`) (45 tests, ~25 s, temp folders only; `AIX_TEST_NETWORK=1` adds the registry downloads; CI runs it on Linux, macOS and Windows, `.github/workflows/aix-tests.yml`). See `tests/README.md`. The manual smoke tests below are what the suite automates.
