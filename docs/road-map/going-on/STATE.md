...
- Kit 1.5.4 (2026-09-06): `aix graph --clones`. 1.5.3: `aix graph --dead`. 1.5.2: README modularity section, graph options in help. 1.5.1: `aix graph` measurement fixed (stability, contracted cycles, upward deps, NCCD, Q, selftest). 1.5.0 on 2026-09-06: `aix graph`/`complexity` (modularity metric), per-command help, kit scripts made acyclic. 1.4.0 earlier the same day: CLI, upgrade, security gate, modularity, Gemini/Antigravity.
