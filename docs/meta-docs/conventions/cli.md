...
| `aix graph` / `aix complexity` `[PATH...] [--functions] [--gate] [--max-reducible PCT] [--report]` | The modularity metric: dependency graph (modules for Python/JS/TS/Rust/Java, `--functions` for Python calls), facades collapsed, leaves set aside; complexity vs ideal complexity (transitive reduction) = **reducible %** (shortcuts + cycle edges; 0 % = ideal), plus cycles, hubs, propagation cost, diamond shape. `--gate` fails on any cycle or reducible above `--max-reducible`; `--report` writes `docs/tests/dependency-graph.md` | Design reviews; `review-code-review` on every diff; CI gate |
