...
never mark generic helpers. `aix docs coverage` scans them (no LLM, no full read), and `aix docs validate` fails any doc whose status claims `implemented` / `automated` / `mitigated` without the marker in code.
