...
Inputs: user intent; `docs/requirements/product/glossary.md` (domain codes); `docs/requirements/how-to-write-requirements.md`; existing IDs in the domain folder INDEX. Templates in `.aix/templates/`.
...
3. Draft from `.aix/templates/requirement.md`: one SHALL statement, ACs in Given/When/Then, constraints, out-of-scope; `status: draft`.
...
5. If the requirement implies a boundary or data: draft `API-*` (`.aix/templates/api-contract.md`) and/or `DM-*` (`.aix/templates/data-model.md`) stubs and link them in `related:`.
