...
2. Categories touched → skills (map in audit-process.md). Always include `security-audit-dependencies` if lockfiles changed, `security-audit-licenses` if a dependency was added and `security-audit-secrets-config` if config changed.
