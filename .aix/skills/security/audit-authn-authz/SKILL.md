...
Read: register rows for categories AUTHN and AUTHZ (`grep -n "VUL-AUTHN\|VUL-AUTHZ" docs/security/vulnerability-register.md`); `.aix/meta-docs/security/secure-coding-baseline.md`; relevant meta-doc: `architecture/layering.md` (policy in services).
...
## Procedure
1. Locate code by layer (`.aix/meta-docs/architecture/project-layout.md`): `backend/app/auth/`, every `services/` file for object checks, `schemas/` for mass assignment. Grep targeted patterns; do not read whole directories.
