...
Read: register rows for category WEB (`grep -n "VUL-WEB" docs/security/vulnerability-register.md`); `.aix/meta-docs/security/secure-coding-baseline.md`; relevant meta-doc: `architecture/frontend-backend-separation.md`.
...
## Procedure
1. Locate code by layer (`.aix/meta-docs/architecture/project-layout.md`): `frontend/src/`, `backend/app/*/views/`, `core/` middleware, `infra/` edge config. Grep targeted patterns; do not read whole directories.
