...
| `aix code licenses [--gate] [--report]` | The licence of every installed dependency (Python, npm, Cargo, Maven) from the metadata on disk, classed permissive / weak copyleft / strong copyleft / proprietary / unknown; the gate fails while one is undecided |
...
| `aix code sbom [--spdx] [--gate]` (`aix blackduck`) | The software bill of materials, CycloneDX or SPDX, with every installed package, its licence and its advisories with a severity; the gate is the composition policy: no advisory at or above `sbom_max_severity`, no licence undecided |
...
| `aix agent claim\|release\|whoami\|list`, `set total N` | Seats for several agents in one repository: a task is signed by the seat that started it and refused to others while that seat is live; stale seats are detected (dead process, expired heartbeat); one state file per seat |
| `aix policy [list\|show\|use\|off]`, `aix check` | The development cycle as an opt-in policy: minimal, standard, hotfix, release, or anarchy (the default: nothing checked; also none, nothing, freedom). Checks run in order, `aix task done` refuses to close while a required one fails, the steps are listed in AGENTS.md |
